...
export function StaticMap({ pins, centerLat, centerLng, zoom = 12, width = 800, height = 400 }: StaticMapProps) {
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden border border-hela-dark/5">
      <img src={url} alt="Mapa de heladerias" width={width} height={height} className="w-full h-auto" loading="lazy" />
      <a href={mapsLink} target="_blank" rel="noopener noreferrer" className="absolute bottom-3 right-3 bg-white/90 backdrop-blur-sm text-hela-dark text-xs font-medium px-3 py-1.5 rounded-full shadow-sm hover:bg-white transition-colors">Abrir en Google Maps</a>
    </div>
  );
}
